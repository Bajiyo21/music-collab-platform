...
import { Music, Users, Heart, Plus, TrendingUp, Clock, Settings, LogOut } from "lucide-react";
import { useState } from "react";

/**
 * Dashboard Page
 * Main hub for authenticated users to manage tracks, collaborations, and playlists
 */
...
const MOCK_RECENT_TRACKS: RecentTrack[] = [
  {
    id: 1,
    title: "Neon Dreams",
    genre: "Synthwave",
...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "tracks" | "collaborations" | "playlists">(
    "overview"
  );
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-black/40 backdrop-blur-md">
        <div className="container flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <a href="/dashboard" className="text-sm text-cyan-400 transition font-semibold">
              Dashboard
            </a>
          </nav>
        ...
        <div className="container">
          ...
          <div className="mb-12 bg-gradient-to-r from-cyan-500/10 to-magenta-500/10 rounded-lg p-8 border border-white/10">
            ...
            <p className="text-muted-foreground mb-6">
              Last active {stats.recentActivity} • Ready to create something amazing?
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
            ...
            <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded p-6 hover:border-magenta-400/50 transition-all">
              ...
              <div className="text-3xl font-bold">{stats.totalCollaborations}</div>
          ...
          <div className="mb-8 border-b border-white/10">
            <div className="flex gap-8">
              {(["overview", "tracks", "collaborations", "playlists"] as const).map((tab) => (
                <button
                  key={tab}
              ...
              <div className="lg:col-span-2 bg-black/40 backdrop-blur-md border border-white/10 rounded p-6">
                ...
                <div className="space-y-4">
                  {MOCK_RECENT_TRACKS.map((track) => (
                    <div
                      key={track.id}
                      className="flex items-center justify-between p-4 bg-black/20 rounded hover:bg-black/40 transition-all cursor-pointer group"
                    >
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div>
                          <h3 className="font-semibold group-hover:text-cyan-400 transition">{track.title}</h3>
                      ...
                      <div className="flex items-center gap-6 text-sm text-muted-foreground">
                        ...
                        <span className="text-xs">{track.uploadedAt}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {MOCK_RECENT_TRACKS.map((track) => (
                  <div
                    key={track.id}
                    className="bg-black/20 rounded p-4 hover:border-cyan-400/50 border border-white/10 transition-all group cursor-pointer"
                  >
                    ...
                    <h3 className="font-bold mb-2 group-hover:text-cyan-400 transition">{track.title}</h3>
          ...
          {activeTab === "collaborations" && (
            <div className="text-center py-12 bg-black/40 backdrop-blur-md border border-white/10 rounded p-6">
              ...
              <p className="text-muted-foreground mb-6">You have 5 active collaborations</p>
          ...
          {activeTab === "playlists" && (
            <div className="text-center py-12 bg-black/40 backdrop-blur-md border border-white/10 rounded p-6">
              ...
              <p className="text-muted-foreground mb-6">You have 8 playlists</p>
